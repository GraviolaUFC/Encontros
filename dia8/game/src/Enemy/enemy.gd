...
@onready var sprite_shader: ShaderMaterial = $AnimatedSprite2D.material
...
func _damage() -> void:
	# Animação de dano
	sprite_shader.set_shader_parameter("amount", 1.0)
	create_tween().tween_method(func(a):
		sprite_shader.set_shader_parameter("amount", a)
		, 1.0, 0.0, 0.25)
